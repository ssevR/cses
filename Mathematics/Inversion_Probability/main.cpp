...
bigint bpow(bigint a, int p) {
    if (p == 0) return 1; bigint tmp = bpow(a, p/2);
    tmp *= tmp;
    if (p % 2)
        tmp *= a;
    return tmp;
    ...
    for (int i = 0; i < n; ++i)
        for (int j = i + 1; j < n; ++j)
            if (r[i] <= r[j]) {
                ...
                s += (prod/r[i]) * ((r[i] - r[j]) * 2 + r[j] - 1);
            }
 
    prod *= 2;
    cout << s/prod;
    auto drop = (s % prod) * bpow(10, P + 6) / prod;
    if (drop % bpow(10, P) > bpow(10, P - 1) * 5) {
        drop /= bpow(10, P);
        drop += 1;
    }
    else if (drop % bpow(10, P) < bpow(10, P - 1) * 5){
        drop /= bpow(10, P);
    }
    else {
        drop /= bpow(10, P);
